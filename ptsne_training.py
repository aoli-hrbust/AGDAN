...
from torch_utils import EPS_like, EPS_max
...
def calculate_optimized_p_cond(
    ...
    diag_mask = (1 - eye(n_points)).to(device)

    # Initializing sigmas
    min_sigma_sq = (min_allowed_sig_sq + 1e-20) * ones(n_points).to(device)
    max_sigma_sq = max_allowed_sig_sq * ones(n_points).to(device)
...
def get_q_joint(
        emb_points: torch.Tensor, dist_func: str = "euc", alpha: int = 1, mask_diag=True
) -> torch.Tensor:
    """
    Calculates the joint probability matrix in embedding space.
    :param emb_points: Points in embeddings space (Tensor)
    :param alpha: Number of degrees of freedom in t-distribution
    :param dist_func: A key name for a distance function
    :param mask_diag: Whether to mask the diagonal of the matrix
    :return: Joint distribution matrix in embedding space
    """
    # Move embeddings to the device
    emb_points = emb_points.to(device)

    # Get the distance function
    dist_f = distance_functions[dist_func]

    # Compute pairwise distances and move to GPU
    distances = dist_f(emb_points) / alpha

    # Efficient calculation of the joint probability matrix
    q_joint = (1 + distances).pow(-(1 + alpha) / 2)

    if mask_diag:
        # Create the mask on the GPU
        n_points = emb_points.size(0)
        mask = torch.eye(n_points, device=device).bool()  # Ensure mask is on the GPU
        q_joint = q_joint.masked_fill(mask, 0)  # Mask diagonal elements

    # Normalize the matrix on GPU
    q_joint = q_joint / q_joint.sum()

    # Ensure EPS_max also runs on GPU
    ...
    #     # 创建掩码并转移到 GPU
    #     n_points = emb_points.size(0)
    #     mask = (-torch.eye(n_points, device=device) + 1).to(device)
